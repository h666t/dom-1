...
window.dom = {
  create: function create(string) {
    var container = document.createElement("template"); //template可以容纳任何东西，防止有些不太准确的语法无法被html识别

    container.innerHTML = string.trim(); //trim 防止空格影响代码

    return container.content.firstChild;
  },
  after: function after(node, node2) {
    node.parentNode.insertBefore(node2, node.nextSibling); //第一个参数为节点，第二个参数为内容
  },
  ...
  wrap: function wrap(node, parent) {
    //给node一个parent
    dom.before(node, parent); //先让node和parent称为sibling

    dom.append(parent, node); //在将node变成parent的child
  },
